...
if __name__ == "__main__":  #  Prevent multiprocessing issues
    ...
    for trial in range(num_trials):
        ...
        model, optimizer, val_acc = train_model(lr=lr, batch_size=batch_size, optimizer_name=optimizer_name, epochs=10)
...
# # Training function with logging
    # def train_model(lr=0.001, batch_size=32, optimizer_name="adam", epochs=10):
    #     """Train the model with given hyperparameters and return validation accuracy."""
    ...
    # print(f"Validation set size: {len(val_dataset)}, Validation loader batches: {len(val_loader)}")
    ...
    #     # Train and get validation accuracy
    #     model = train_model(lr=lr, batch_size=batch_size, optimizer_name=optimizer_name, epochs=10)
    ...
    #         best_model_state = model.state_dict()  # Save model state dictionary
    ...
    # if best_model is not None:
    #     os.makedirs(CHECKPOINT_DIR, exist_ok=True)  # Ensure directory exists
    #     torch.save({"model_state_dict": best_model.state_dict(), "params": best_params}, CHECKPOINT_PATH)
    #     print(f"Best model saved successfully at {CHECKPOINT_PATH}")
    # else:
    #     print("No best model found.")